...
export function EditEntryModal({ entry, onClose }: EditEntryModalProps) {
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: ["history", "foods"] });
    ...
    useEffect(() => {
        ...
        form.reset({
            type: "food",
            consumed_at: fromIsoToLocalInput(entry.consumed_at),
            food_name: entry.food_name,
            meal_category_code: entry.meal_category_code,
            notes: entry.notes ?? "",
            quantity_unit: entry.quantity_unit ?? "",
            quantity_value: entry.quantity_value !== null ? String(entry.quantity_value) : ""
        });
        setFormError(null);
    }, [entry, form]);
    ...
    return (
        <Modal onClose={onClose} open={Boolean(entry)} title="Editar alimento">
            <form
                className="space-y-4"
            ...
            >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <input
                            ...
                            onChange={(e) =>
                                form.setValue(
                                    "consumed_at",
                                    mergeLocalDateTime(e.target.value, consumedTime),
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                form.setValue(
                                    "consumed_at",
                                    mergeLocalDateTime(consumedDate, e.target.value),
                                    { shouldDirty: true, shouldValidate: true }
                                )
                            }
                        />
                    </div>
                    <input type="hidden" {...form.register("consumed_at")} />
                </Field>
                ...
                <button
                    className="w-full rounded-xl border border-(--color-error) py-2.5 text-sm font-semibold text-(--color-error) transition-colors hover:bg-(--color-error-bg) disabled:opacity-50"
                    disabled={mutation.isPending || deleteMutation.isPending}
                    onClick={() => {
                    ...
                    type="button"
                >
                    {deleteMutation.isPending ? "Eliminando..." : "Eliminar entrada"}
                </button>
            </form>
        </Modal>
    );
